### Задача: Создайте программу для игры в ""Крестики-нолики"".
...
while True:
    ...
    def draw_board():
        res = ""
        for i, v in enumerate(board):     #Возвращает перечисляемый объект. iterable должен быть последовательностью, итератором или другим объектом, поддерживающим итерацию.
            res += str(v) + " "
            if (i + 1) % BOARD_SIZE == 0:
                res += "\n"
        print(res)
    ...
    def validate(value=" "):
        if not value.isdigit() and int(value) not in available_turns:
            raise ValueError("Enter valid value and try again")   #исключение ValueError:  Возникает, когда операция
                                                                # или функция получает аргумент правильного типа,
                                                                # но с неподходящим значением, и ситуация не описывается более точным исключением, таким как IndexError.
    ...
    for i in range(9):
        try:                      #Оператор попытки: Оператор try указывает обработчики исключений и/или код очистки для группы операторов
            input_error = True
            draw_board()
            while input_error:
